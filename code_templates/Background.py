class Background_template:
    ...
    def gen_init_func(self):
        ...
        self.init_func += f"\n"
        self.init_func += '        with open(__file__.replace(os.path.basename(__file__), "")+"/img/bg.json") as f:\n'
        self.init_func += f"            self.info = json.load(f)\n"
        self.init_func += f"\n"

    def gen_build_surf_func(self):
        self.surf_func = f"    def build_surf(self, filename):\n"
        self.surf_func += (
            f"        im_surf = pygame.image.load(filename).convert_alpha()\n"
        )
        self.surf_func += f"        self.surf = pygame.Surface((3*im_surf.get_width(), im_surf.get_height()))\n"
        self.surf_func += f"\n"
        self.surf_func += f"        for i in range(self.n_imgs):\n"
        self.surf_func += f"            # TODO Generalize for the case where we have other background\n"
        self.surf_func += f"            # images and not the same repeated image.\n"
        self.surf_func += (
            f"            self.surf.blit(im_surf, (self.x+i*im_surf.get_width(), 0))\n"
        )
        self.surf_func += f"\n"
        self.surf_func += "        self.im_dims = {'w':im_surf.get_width(), 'h':im_surf.get_height()}\n"
        self.surf_func += f"\n"

    def build_update_func(self):
        ...
        self.update_func += (
            f"                # images, we may need to actually load the next one.\n"
        )
        self.update_func += f"                self.x -= self.im_dims['w']\n"
        self.update_func += f"                self.repeat -= 1\n"
        self.update_func += '                print(f"repeat = {self.repeat}")\n'
        ...
        self.update_func += f"                # TODO Generalize for the case where we have other background\n"
        self.update_func += (
            f"                # images and not the same image repeating.\n"
        )
        self.update_func += f"                self.x += self.im_dims['w']\n"
        self.update_func += f"                self.repeat += 1\n"
        self.update_func += '                print(f"repeat = {self.repeat}")\n'
    ...
    def write_class_to_file(self, filename="background.py"):
        ...
        self.build_update_func()

        with open(filename, "w") as g:
            g.write(self.init_func)
            g.write(self.surf_func)
